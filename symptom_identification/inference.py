...
def main(config):
    ...
    # symptom labels
    label_index = torch.zeros((data.shape[0], num_labels))
    ...
    X = data['pre_question'].values
    test_dataset = module_data.MultiLabelDataLoader(X, label_index, tokenizer, max_length) 
    ...
    ## error analysis
    text = data['pre_question'].values # inference
    ...
    res_df.to_csv(str(config.log_dir)+'/error_analysis_all.csv', index=False)
    
    # state vector, score vector를 .pkl 파일로 저장
    with open(str(config.log_dir)+'/uncertainty_'+str(fold)+'.pkl', 'wb') as f:
        pickle.dump(ood_preds, f)

    with open(str(config.log_dir)+'/symptom_scores_'+str(fold)+'_all.pkl', 'wb') as f:
        pickle.dump(all_symptom_scores, f)
